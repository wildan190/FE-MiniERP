  ...
  <AppLayout>
    <div class="max-w-7xl mx-auto px-4 py-8">
      <div v-if="loading" class="flex flex-col items-center justify-center py-20">
        ...
        <p class="mt-4 text-gray-500 font-medium">Loading designation details...</p>
      </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div class="flex items-center gap-4">
            <div class="h-16 w-16 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
              {{ designation.name?.charAt(0).toUpperCase() || 'D' }}
            </div>
            <div>
              <h1 class="text-3xl font-bold text-gray-900">{{ designation.name }}</h1>
              <div class="flex flex-wrap items-center gap-2 mt-1">
                <span class="text-gray-600 text-sm">Created {{ formatDate(designation.created_at) }}</span>
                <span class="text-gray-400">•</span>
                <span class="text-gray-600 text-sm">UUID: {{ designation.uuid }}</span>
              </div>
            </div>
          </div>
            ...
            <Card>
              ...
              <div class="space-y-6">
                <div>
                  ...
                  <p class="text-gray-900 font-medium text-lg">{{ designation.name }}</p>
                </div>
                <div>
                  <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Description</label>
                  <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">{{ designation.description || 'No description provided' }}</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            <Card>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-sm font-medium text-gray-900">{{ designation.id }}</p>
                </div>
                <div>
                  <label class="text-xs font-semibold text-gray-500 uppercase block mb-1">UUID</label>
                  <code class="text-[10px] bg-gray-50 p-1 rounded text-gray-600 break-all">{{ designation.uuid }}</code>
                </div>
                <div>
                  <label class="text-xs font-semibold text-gray-500 uppercase block mb-1">Created At</label>
                  <p class="text-sm text-gray-900">{{ formatDate(designation.created_at) }}</p>
                </div>
                <div>
                  <label class="text-xs font-semibold text-gray-500 uppercase block mb-1">Last Updated</label>
                  <p class="text-sm text-gray-900">{{ formatDate(designation.updated_at) }}</p>
                </div>
              </div>
            </Card>
          </div>
      ...
      <div v-else class="text-center py-20 bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <h2 class="text-xl font-bold text-gray-900">Designation Not Found</h2>
        <p class="text-gray-500 mt-2">The designation you are looking for might have been moved or deleted.</p>
        <router-link
          to="/hrm/designations"
      ...
      <CreateDesignationModal
        ...
        :editing-designation="designation"
        :errors="submitErrors"
...
import type { CreateDesignationRequest } from '../../services/hrm/types/designation.types'
...
const designation = computed(() => designationStore.currentDesignation)
...
const fetchDesignation = async () => {
  ...
  try {
    ...
    await designationStore.fetchDesignationDetail(uuid)
  } catch (error) {
...
const handleUpdate = async (data: CreateDesignationRequest) => {
  ...
  try {
    await designationStore.updateDesignation(designation.value.uuid, data)
    await Swal.fire({
      title: 'Success!',
    ...
    if (error.response?.status === 422) {
      ...
      submitErrorMessage.value = error.response?.data?.message || 'Failed to update designation. Please try again.'
      Swal.fire({
        title: 'Error!',
        text: submitErrorMessage.value || 'Failed to update designation',
        icon: 'error',
...
const handleDelete = async () => {
  ...
  const result = await Swal.fire({
    ...
    text: `Are you sure you want to delete "${designation.value.name}"? This action cannot be undone.`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await designationStore.deleteDesignation(designation.value.uuid)
      await Swal.fire({
        title: 'Deleted!',
