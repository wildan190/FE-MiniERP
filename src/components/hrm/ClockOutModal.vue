    ...
    <div class="flex items-end justify-center min-h-screen px-2 py-4 sm:px-4 sm:py-0 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-full max-w-md p-6">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="text-center p-6 bg-amber-50 rounded-xl border border-amber-100">
            <p class="text-amber-800 text-sm">Are you sure you want to clock out for today?</p>
            <p class="text-[10px] text-amber-600 mt-1 uppercase tracking-wider font-bold">End of shift</p>
          </div>

          <!-- Face Verification (Optional for Clock Out usually, but supported by API) -->
          <div class="space-y-2">
            <label class="block text-sm font-medium text-gray-700">Face Verification (Optional)</label>
            <div class="aspect-video bg-gray-100 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center overflow-hidden relative">
              ...
              <div v-if="isCameraActive && !imagePreview" class="absolute bottom-4 left-0 right-0 flex justify-center">
                ...
                >
                  <div class="h-6 w-6 rounded-full border-4 border-amber-600"></div>
                </button>
              </div>
          ...
          <!-- Notes -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Clock Out Notes</label>
            <textarea
              v-model="form.notes"
              rows="2"
              class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all resize-none"
